...
const login = asyncHandler(async (req, res) => {
  ...
  if (user.twoFactorEnabled) {
    if (!twoFactorCode) throw new AppError('2FA code required', 401, '2FA_REQUIRED');
    let valid = authenticator.verify({ token: twoFactorCode, secret: user.twoFactorSecret });

    // Fall back to backup code (compares hash, marks as used on success)
    if (!valid) {
      const crypto = require('crypto');
      const inputHash = crypto.createHash('sha256').update(twoFactorCode.toUpperCase().replace(/-/g, '')).digest('hex');
  ...
  // Detect new device — alert by email if we've never seen this UA before.
  const isNewDevice = !user.refreshTokens.some((t) => t.deviceInfo === incomingUA);

  // Atomic $push + $slice keeps the array capped at 5 even when N concurrent
  // logins race — pre-fix the splice-then-save pattern could end up with up
  // to N tokens because each request read len<5 before pushing.
  await User.updateOne(
    { _id: user._id },
    {
      $push: {
